    ...
    implements ExtendedDigest, Memoable
{
    ...
    public TigerDigest(TigerDigest tigerdigest)
    {
        ...
        apa = new long[8];
        anI = 0;
        _mth02CA(tigerdigest);
    }

    private void _mth02CA(long l, long l1)
    {
        aoY = aoY ^ l;
        aoW = aoW - (aoS[(int)aoY & 0xff] ^ aoT[(int)(aoY >> 16) & 0xff] ^ aoU[(int)(aoY >> 32) & 0xff] ^ aoV[(int)(aoY >> 48) & 0xff]);
        aoX = aoX + (aoV[(int)(aoY >> 8) & 0xff] ^ aoU[(int)(aoY >> 24) & 0xff] ^ aoT[(int)(aoY >> 40) & 0xff] ^ aoS[(int)(aoY >> 56) & 0xff]);
        aoX = aoX * l1;
    }

    private static void _mth02CA(long l, byte abyte0[], int i)
    {
    ...
    private void _mth02CB(long l, long l1)
    {
        aoW = aoW ^ l;
        aoX = aoX - (aoS[(int)aoW & 0xff] ^ aoT[(int)(aoW >> 16) & 0xff] ^ aoU[(int)(aoW >> 32) & 0xff] ^ aoV[(int)(aoW >> 48) & 0xff]);
        aoY = aoY + (aoV[(int)(aoW >> 8) & 0xff] ^ aoU[(int)(aoW >> 24) & 0xff] ^ aoT[(int)(aoW >> 40) & 0xff] ^ aoS[(int)(aoW >> 56) & 0xff]);
        aoY = aoY * l1;
    }

    private void _mth02CE(long l, long l1)
    {
        aoX = aoX ^ l;
        aoY = aoY - (aoS[(int)aoX & 0xff] ^ aoT[(int)(aoX >> 16) & 0xff] ^ aoU[(int)(aoX >> 32) & 0xff] ^ aoV[(int)(aoX >> 48) & 0xff]);
        aoW = aoW + (aoV[(int)(aoX >> 8) & 0xff] ^ aoU[(int)(aoX >> 24) & 0xff] ^ aoT[(int)(aoX >> 40) & 0xff] ^ aoS[(int)(aoX >> 56) & 0xff]);
        aoW = aoW * l1;
    }
    ...
    private void _mth141D(byte abyte0[], int i)
    {
        ...
        int j = anI;
        anI = j + 1;
        al[j] = (long)(abyte0[i + 7] & 0xff) << 56 | (long)(abyte0[i + 6] & 0xff) << 48 | (long)(abyte0[i + 5] & 0xff) << 40 | (long)(abyte0[i + 4] & 0xff) << 32 | (long)(abyte0[i + 3] & 0xff) << 24 | (long)(abyte0[i + 2] & 0xff) << 16 | (long)(abyte0[i + 1] & 0xff) << 8 | (long)(abyte0[i] & 0xff);
        if (anI == apa.length)
        {
            _mth14BA();
        }
    ...
    private void _mth1602()
    {
        long al[] = apa;
        al[0] = al[0] - (apa[7] ^ 0xa5a5a5a5a5a5a5a5L);
        al = apa;
        al[1] = al[1] ^ apa[0];
        al = apa;
        al[2] = al[2] + apa[1];
        al = apa;
        al[3] = al[3] - (apa[2] ^ ~apa[1] << 19);
        al = apa;
        al[4] = al[4] ^ apa[3];
        al = apa;
        al[5] = al[5] + apa[4];
        al = apa;
        al[6] = al[6] - (apa[5] ^ ~apa[4] >>> 23);
        al = apa;
        al[7] = al[7] ^ apa[6];
        al = apa;
        al[0] = al[0] + apa[7];
        al = apa;
        al[1] = al[1] - (apa[0] ^ ~apa[7] << 19);
        al = apa;
        al[2] = al[2] ^ apa[1];
        al = apa;
        al[3] = al[3] + apa[2];
        al = apa;
        al[4] = al[4] - (apa[3] ^ ~apa[2] >>> 23);
        al = apa;
        al[5] = al[5] ^ apa[4];
        al = apa;
        al[6] = al[6] + apa[5];
        al = apa;
        al[7] = al[7] - (apa[6] ^ 0x123456789abcdefL);
    }
    ...
    public final int doFinal(byte abyte0[], int i)
    {
        ...
        apa[7] = l << 3;
        _mth14BA();
        _mth02CA(aoW, abyte0, i);
        _mth02CA(aoX, abyte0, i + 8);
        _mth02CA(aoY, abyte0, i + 16);
        reset();
        return 24;
    }
    ...
    {
        aoW = 0x123456789abcdefL;
        aoX = 0xfedcba9876543210L;
        aoY = 0xf096a5b4c3b2e187L;
        anI = 0;
        for (int i = 0; i != apa.length; i++)
        {
            apa[i] = 0L;
    ...
    {
        memoable = (TigerDigest)memoable;
        aoW = ((TigerDigest) (memoable)).aoW;
        aoX = ((TigerDigest) (memoable)).aoX;
        aoY = ((TigerDigest) (memoable)).aoY;
        System.arraycopy(((TigerDigest) (memoable)).apa, 0, apa, 0, ((TigerDigest) (memoable)).apa.length);
        anI = ((TigerDigest) (memoable)).anI;
        System.arraycopy(((TigerDigest) (memoable)).buf, 0, buf, 0, ((TigerDigest) (memoable)).buf.length);
        aoZ = ((TigerDigest) (memoable)).aoZ;
        VD = ((TigerDigest) (memoable)).VD;
    }
