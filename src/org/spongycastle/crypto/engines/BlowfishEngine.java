    ...
    implements BlockCipher
{
    ...
    private final int aqn[] = new int[256];
    private final int aqo[] = new int[256];
    private final int aqp[] = new int[256];
    private final int aqq[] = new int[256];
    private final int aqr[] = new int[18];
    private boolean aqs;
    private byte aqt[];
    ...
    private void _mth02CA(int i, int j, int ai[])
    {
        int i1 = ai.length;
        for (int k = 0; k < i1; k += 2)
        {
            ...
            boolean flag = true;
            int l = j;
            for (j = ((flag) ? 1 : 0); j < 16; j += 2)
            {
                l ^= _mth4E36(i) ^ aqr[j];
                i ^= _mth4E36(l) ^ aqr[j + 1];
            }
    ...
    private void _mth02CB(byte abyte0[], int i, byte abyte1[], int j)
    {
        ...
        int l = _mth037A(abyte0, i + 4);
        k ^= aqr[0];
        for (i = 1; i < 16; i += 2)
        {
            l ^= _mth4E36(k) ^ aqr[i];
            k ^= _mth4E36(l) ^ aqr[i + 1];
        }

        _mth02CB(aqr[17] ^ l, abyte1, j);
        _mth02CB(k, abyte1, j + 4);
    }
    ...
    private void _mth02CE(byte abyte0[], int i, byte abyte1[], int j)
    {
        ...
        _mth02CB(aqr[0] ^ l, abyte1, j);
        _mth02CB(k, abyte1, j + 4);
    }

    private static int _mth037A(byte abyte0[], int i)
    {
        return (abyte0[i] & 0xff) << 24 | (abyte0[i + 1] & 0xff) << 16 | (abyte0[i + 2] & 0xff) << 8 | abyte0[i + 3] & 0xff;
    }

    private int _mth4E36(int i)
    {
        return (aqn[i >>> 24] + aqo[i >>> 16 & 0xff] ^ aqp[i >>> 8 & 0xff]) + aqq[i & 0xff];
    }
    ...
    public final void _mth02CA(boolean flag, CipherParameters cipherparameters)
    {
        if (cipherparameters instanceof KeyParameter)
        {
            ...
            cipherparameters = aqt;
            System.arraycopy(aqj, 0, aqn, 0, 256);
            System.arraycopy(aqk, 0, aqo, 0, 256);
            System.arraycopy(aql, 0, aqp, 0, 256);
            System.arraycopy(aqm, 0, aqq, 0, 256);
            System.arraycopy(aqi, 0, aqr, 0, 18);
            int j1 = cipherparameters.length;
            int i = 0;
            for (int j = 0; j < 18; j++)
            {
                int l = 0;
            ...
            _mth02CA(0, 0, aqr);
            _mth02CA(aqr[16], aqr[17], aqn);
            _mth02CA(aqn[254], aqn[255], aqo);
            _mth02CA(aqo[254], aqo[255], aqp);
            _mth02CA(aqp[254], aqp[255], aqq);
            return;
        } else
        {
            throw new IllegalArgumentException((new StringBuilder("invalid parameter passed to Blowfish init - ")).append(cipherparameters.getClass().getName()).toString());
        }
    }
