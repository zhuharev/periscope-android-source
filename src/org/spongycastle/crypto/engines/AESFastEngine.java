    ...
    implements BlockCipher
{
    ...
    private void _mth02CA(int ai[][])
    {
        ...
        int j2;
        int l2;
        for (i1 = aqg ^ ai[0][3]; i < aqb - 1; i1 = l2 ^ j2 ^ l1 ^ j1 ^ ai[i1][3])
        {
            j2 = aoA[l & 0xff] ^ aoA[(k >>> 8 & 0xff) + 256] ^ aoA[(j >>> 16 & 0xff) + 512] ^ aoA[(i1 >>> 24 & 0xff) + 768] ^ ai[i][0];
            l1 = aoA[k & 0xff] ^ aoA[(j >>> 8 & 0xff) + 256] ^ aoA[(i1 >>> 16 & 0xff) + 512] ^ aoA[(l >>> 24 & 0xff) + 768] ^ ai[i][1];
            j1 = aoA[j & 0xff] ^ aoA[(i1 >>> 8 & 0xff) + 256] ^ aoA[(l >>> 16 & 0xff) + 512] ^ aoA[(k >>> 24 & 0xff) + 768] ^ ai[i][2];
            l2 = aoA[i1 & 0xff];
            l = aoA[(l >>> 8 & 0xff) + 256];
            k = aoA[(k >>> 16 & 0xff) + 512];
            j = aoA[(j >>> 24 & 0xff) + 768];
            i1 = i + 1;
            i = l2 ^ l ^ k ^ j ^ ai[i][3];
            l = aoA[j2 & 0xff] ^ aoA[(l1 >>> 8 & 0xff) + 256] ^ aoA[(j1 >>> 16 & 0xff) + 512] ^ aoA[(i >>> 24 & 0xff) + 768] ^ ai[i1][0];
            k = aoA[l1 & 0xff] ^ aoA[(j1 >>> 8 & 0xff) + 256] ^ aoA[(i >>> 16 & 0xff) + 512] ^ aoA[(j2 >>> 24 & 0xff) + 768] ^ ai[i1][1];
            j = aoA[j1 & 0xff] ^ aoA[(i >>> 8 & 0xff) + 256] ^ aoA[(j2 >>> 16 & 0xff) + 512] ^ aoA[(l1 >>> 24 & 0xff) + 768] ^ ai[i1][2];
            l2 = aoA[i & 0xff];
            j2 = aoA[(j2 >>> 8 & 0xff) + 256];
            l1 = aoA[(l1 >>> 16 & 0xff) + 512];
            j1 = aoA[(j1 >>> 24 & 0xff) + 768];
            i = i1 + 1;
        }

        int k1 = aoA[l & 0xff] ^ aoA[(k >>> 8 & 0xff) + 256] ^ aoA[(j >>> 16 & 0xff) + 512] ^ aoA[(i1 >>> 24 & 0xff) + 768] ^ ai[i][0];
        int i2 = aoA[k & 0xff] ^ aoA[(j >>> 8 & 0xff) + 256] ^ aoA[(i1 >>> 16 & 0xff) + 512] ^ aoA[(l >>> 24 & 0xff) + 768] ^ ai[i][1];
        int k2 = aoA[j & 0xff] ^ aoA[(i1 >>> 8 & 0xff) + 256] ^ aoA[(l >>> 16 & 0xff) + 512] ^ aoA[(k >>> 24 & 0xff) + 768] ^ ai[i][2];
        i1 = aoA[i1 & 0xff];
        l = aoA[(l >>> 8 & 0xff) + 256];
        k = aoA[(k >>> 16 & 0xff) + 512];
        int i3 = aoA[(j >>> 24 & 0xff) + 768];
        j = i + 1;
        i = i1 ^ l ^ k ^ i3 ^ ai[i][3];
        k = anp[k1 & 0xff];
        aqd = (anp[i2 >>> 8 & 0xff] & 0xff) << 8 ^ k & 0xff ^ (anp[k2 >>> 16 & 0xff] & 0xff) << 16 ^ (anp[i >>> 24 & 0xff] & 0xff) << 24 ^ ai[j][0];
        k = anp[i2 & 0xff];
        aqe = (anp[k2 >>> 8 & 0xff] & 0xff) << 8 ^ k & 0xff ^ (anp[i >>> 16 & 0xff] & 0xff) << 16 ^ (anp[k1 >>> 24 & 0xff] & 0xff) << 24 ^ ai[j][1];
        k = anp[k2 & 0xff];
        aqf = (anp[i >>> 8 & 0xff] & 0xff) << 8 ^ k & 0xff ^ (anp[k1 >>> 16 & 0xff] & 0xff) << 16 ^ (anp[i2 >>> 24 & 0xff] & 0xff) << 24 ^ ai[j][2];
        i = anp[i & 0xff];
        aqg = (anp[k1 >>> 8 & 0xff] & 0xff) << 8 ^ i & 0xff ^ (anp[i2 >>> 16 & 0xff] & 0xff) << 16 ^ (anp[k2 >>> 24 & 0xff] & 0xff) << 24 ^ ai[j][3];
    }
    ...
    private void _mth02CB(int ai[][])
    {
        ...
        int j2;
        int k2;
        for (i1 = aqg ^ ai[aqb][3]; k > 1; i1 = k2 ^ j2 ^ l1 ^ j1 ^ ai[i1][3])
        {
            j1 = aqh[i & 0xff] ^ aqh[(i1 >>> 8 & 0xff) + 256] ^ aqh[(l >>> 16 & 0xff) + 512] ^ aqh[(j >>> 24 & 0xff) + 768] ^ ai[k][0];
            l1 = aqh[j & 0xff] ^ aqh[(i >>> 8 & 0xff) + 256] ^ aqh[(i1 >>> 16 & 0xff) + 512] ^ aqh[(l >>> 24 & 0xff) + 768] ^ ai[k][1];
            j2 = aqh[l & 0xff] ^ aqh[(j >>> 8 & 0xff) + 256] ^ aqh[(i >>> 16 & 0xff) + 512] ^ aqh[(i1 >>> 24 & 0xff) + 768] ^ ai[k][2];
            k2 = aqh[i1 & 0xff];
            l = aqh[(l >>> 8 & 0xff) + 256];
            j = aqh[(j >>> 16 & 0xff) + 512];
            i = aqh[(i >>> 24 & 0xff) + 768];
            i1 = k - 1;
            k = k2 ^ l ^ j ^ i ^ ai[k][3];
            i = aqh[j1 & 0xff] ^ aqh[(k >>> 8 & 0xff) + 256] ^ aqh[(j2 >>> 16 & 0xff) + 512] ^ aqh[(l1 >>> 24 & 0xff) + 768] ^ ai[i1][0];
            j = aqh[l1 & 0xff] ^ aqh[(j1 >>> 8 & 0xff) + 256] ^ aqh[(k >>> 16 & 0xff) + 512] ^ aqh[(j2 >>> 24 & 0xff) + 768] ^ ai[i1][1];
            l = aqh[j2 & 0xff] ^ aqh[(l1 >>> 8 & 0xff) + 256] ^ aqh[(j1 >>> 16 & 0xff) + 512] ^ aqh[(k >>> 24 & 0xff) + 768] ^ ai[i1][2];
            k2 = aqh[k & 0xff];
            j2 = aqh[(j2 >>> 8 & 0xff) + 256];
            l1 = aqh[(l1 >>> 16 & 0xff) + 512];
            j1 = aqh[(j1 >>> 24 & 0xff) + 768];
            k = i1 - 1;
        }

        k = aqh[i & 0xff] ^ aqh[(i1 >>> 8 & 0xff) + 256] ^ aqh[(l >>> 16 & 0xff) + 512] ^ aqh[(j >>> 24 & 0xff) + 768] ^ ai[1][0];
        int k1 = aqh[j & 0xff] ^ aqh[(i >>> 8 & 0xff) + 256] ^ aqh[(i1 >>> 16 & 0xff) + 512] ^ aqh[(l >>> 24 & 0xff) + 768] ^ ai[1][1];
        int i2 = aqh[l & 0xff] ^ aqh[(j >>> 8 & 0xff) + 256] ^ aqh[(i >>> 16 & 0xff) + 512] ^ aqh[(i1 >>> 24 & 0xff) + 768] ^ ai[1][2];
        i = aqh[i1 & 0xff] ^ aqh[(l >>> 8 & 0xff) + 256] ^ aqh[(j >>> 16 & 0xff) + 512] ^ aqh[(i >>> 24 & 0xff) + 768] ^ ai[1][3];
        j = apX[k & 0xff];
        aqd = (apX[i >>> 8 & 0xff] & 0xff) << 8 ^ j & 0xff ^ (apX[i2 >>> 16 & 0xff] & 0xff) << 16 ^ (apX[k1 >>> 24 & 0xff] & 0xff) << 24 ^ ai[0][0];
        j = apX[k1 & 0xff];
        aqe = (apX[k >>> 8 & 0xff] & 0xff) << 8 ^ j & 0xff ^ (apX[i >>> 16 & 0xff] & 0xff) << 16 ^ (apX[i2 >>> 24 & 0xff] & 0xff) << 24 ^ ai[0][1];
        j = apX[i2 & 0xff];
        aqf = (apX[k1 >>> 8 & 0xff] & 0xff) << 8 ^ j & 0xff ^ (apX[k >>> 16 & 0xff] & 0xff) << 16 ^ (apX[i >>> 24 & 0xff] & 0xff) << 24 ^ ai[0][2];
        i = apX[i & 0xff];
        aqg = (apX[i2 >>> 8 & 0xff] & 0xff) << 8 ^ i & 0xff ^ (apX[k1 >>> 16 & 0xff] & 0xff) << 16 ^ (apX[k >>> 24 & 0xff] & 0xff) << 24 ^ ai[0][3];
    }

    private static int _mth207F(int i)
    {
        byte byte0 = anp[i & 0xff];
        return (anp[i >>> 8 & 0xff] & 0xff) << 8 | byte0 & 0xff | (anp[i >>> 16 & 0xff] & 0xff) << 16 | (anp[i >>> 24 & 0xff] & 0xff) << 24;
    }
    ...
    public final int _mth02CA(byte abyte0[], int i, byte abyte1[], int j)
    {
        ...
        aqd = Pack._mth02C6(abyte0, i);
        aqe = Pack._mth02C6(abyte0, i + 4);
        aqf = Pack._mth02C6(abyte0, i + 8);
        aqg = Pack._mth02C6(abyte0, i + 12);
        if (amy)
        {
            _mth02CA(aqc);
        } else
        ...
        Pack._mth02BE(aqd, abyte1, j);
        Pack._mth02BE(aqe, abyte1, j + 4);
        Pack._mth02BE(aqf, abyte1, j + 8);
        Pack._mth02BE(aqg, abyte1, j + 12);
        return 16;
    }
