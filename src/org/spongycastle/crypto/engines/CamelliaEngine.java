    ...
    implements BlockCipher
{
    ...
    private static void _mth02CA(int ai[], int ai1[], int i)
    {
        int j = ai[0] ^ ai1[i + 0];
        j = aqP[j & 0xff] ^ aqR[j >>> 8 & 0xff] ^ aqQ[j >>> 16 & 0xff] ^ aqO[j >>> 24];
        int k = ai[1] ^ ai1[i + 1];
        k = aqO[k & 0xff] ^ aqP[k >>> 8 & 0xff] ^ aqR[k >>> 16 & 0xff] ^ aqQ[k >>> 24];
        ai[2] = ai[2] ^ (j ^ k);
        ai[3] = ai[3] ^ (j ^ k ^ (j >>> 8) + (j << 24));
        j = ai[2] ^ ai1[i + 2];
        j = aqP[j & 0xff] ^ aqR[j >>> 8 & 0xff] ^ aqQ[j >>> 16 & 0xff] ^ aqO[j >>> 24];
        i = ai[3] ^ ai1[i + 3];
        i = aqO[i & 0xff] ^ aqP[i >>> 8 & 0xff] ^ aqR[i >>> 16 & 0xff] ^ aqQ[i >>> 24];
        ai[0] = ai[0] ^ (j ^ i);
        ai[1] = ai[1] ^ (j ^ i ^ (j >>> 8) + (j << 24));
    }
