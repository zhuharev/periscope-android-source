    ...
    implements MessageEncryptor
{
    ...
    public final byte[] _mth06E6(byte abyte0[])
    {
        ...
        asp.nextBytes(abyte1);
        GF2Vector gf2vector = new GF2Vector(afK, asp);
        byte abyte2[] = gf2vector.getEncoded();
        byte abyte4[] = ByteUtils._mth0640(abyte0, abyte1);
        aKp.update(abyte4, 0, abyte4.length);
        abyte4 = new byte[aKp._mth14AF()];
        aKp.doFinal(abyte4, 0);
        Object obj = Conversions._mth02CB(axg, aJP, abyte4);
        byte abyte3[] = McElieceCCA2Primitives._mth02CA((McElieceCCA2PublicKeyParameters)aMa, gf2vector, ((GF2Vector) (obj))).getEncoded();
        obj = new DigestRandomGenerator(new SHA1Digest());
        ((DigestRandomGenerator) (obj)).FB1D(abyte2);
        abyte2 = new byte[abyte0.length + k];
        ((DigestRandomGenerator) (obj)).FF9E(abyte2, 0, abyte2.length);
        for (int i = 0; i < abyte0.length; i++)
        {
            abyte2[i] = (byte)(abyte2[i] ^ abyte0[i]);
        }

        for (int j = 0; j < k; j++)
        {
            int l = abyte0.length + j;
            abyte2[l] = (byte)(abyte2[l] ^ abyte1[j]);
        }

        return ByteUtils._mth0640(abyte3, abyte2);
    }
    ...
    public final byte[] _mth0E40(byte abyte0[])
    {
        ...
        digestrandomgenerator.FB1D(abyte1);
        abyte1 = new byte[k];
        ...
        aKp.update(abyte1, 0, abyte1.length);
        abyte0 = new byte[aKp._mth14AF()];
        aKp.doFinal(abyte0, 0);
        if (!Conversions._mth02CB(axg, aJP, abyte0).equals(gf2vector))
        {
            throw new Exception("Bad Padding: Invalid ciphertext.");
        } else
        {
            return ByteUtils._mth2071(abyte1, k - (afK >> 3))[0];
        }
    }
}
